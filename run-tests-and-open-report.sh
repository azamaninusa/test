...
# Script to run tests in Docker and automatically open the test report
# Usage: ./run-tests-and-open-report.sh [test-filter]
...
# Get test filter from command line or use default
TEST_FILTER="${1:-FullyQualifiedName~GoogleSearchTest}"

echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  VaxCare Test Runner${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
echo ""
echo -e "${BLUE}[2/4]${NC} Running tests in Docker..."
...
if docker run --rm \
    ...
    vaxcare-tests \
    --filter "${TEST_FILTER}" 2>&1 | tee /tmp/test_output.log; then
...
if ./generate-html-report.sh > /dev/null 2>&1; then
